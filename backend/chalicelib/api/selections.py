...
@selections_bp.route("/selections/{selection_id}/products/{product_id}", methods=["DELETE"], cors=True)
@require_admin
def delete_selection_product(selection_id, product_id):
    try:
        with get_session() as session:
            fetch(session, Selection, raise_not_found=True, id=selection_id)
            sp = fetch(session, SelectionProduct, raise_not_found=True, id=product_id)
            delete(session, sp)
        return success_response({"message": "삭제되었습니다."})
    except AppError as e:
        return handle_app_error(e)
